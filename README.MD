...
1.  **Install the prerequisites**
    - Install Rust via `rustup` together with the toolchains used in [CI](./.github/workflows/rust_benchmarks_parallel.yml): `nightly-2025-08-18-aarch64-apple-darwin` (default), `1.97.1` for `binius64`, and `nightly-2025-04-06` for crates such as `nexus` and `cairo-m`. Add the `llvm-tools`, `rustc-dev`, `rustfmt`, and `clippy` components so `cargo bench` matches the workflow in `.github/workflows/rust_benchmarks_parallel.yml`.
