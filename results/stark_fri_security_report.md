...
$$
\mathrm{security}_{\mathrm{old}} = \mathrm{pow\_bits} + \mathrm{queries} \cdot \mathrm{log\_blowup}.
$$
...
$$
\mathrm{pow\_bits} = \mathrm{default\_max\_pow}(\mathrm{num\_variables}, 1).
$$
...
$$
\mathrm{fri\_error}_{\mathrm{old}} = \rho^{\mathrm{queries}}.
$$
