...
pub use crate::bench::{get_constraints, prove, verify, M31SingleConfig};

// Includes the circuits that were generated by '../build.rs'
include!(concat!(env!("OUT_DIR"), "/poseidon_sizes.rs"));

/// Computes Poseidon hash over inputs and returns the state output
pub fn check_poseidon<C: Config, B: RootAPI<C>>(api: &mut B, inputs: &[Variable]) -> Vec<Variable> {
    ...
    params.hash_to_state(api, inputs)
}
